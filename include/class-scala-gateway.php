<?php
/*
 * Main Gateway of Scala using either a local daemon or the explorer
 * Authors: SerHack, cryptochangements, mosu-forge
 */

defined( 'ABSPATH' ) || exit;

require_once('class-scala-cryptonote.php');

class Scala_Gateway extends WC_Payment_Gateway
{
    private static $_id = 'scala_gateway';
    private static $_title = 'Scala Gateway';
    private static $_method_title = 'Scala Gateway';
    private static $_method_description = 'Scala Gateway Plug-in for WooCommerce.';
    ...
    private static $use_scala_price = false;
    private static $use_scala_price_decimals = MONERO_GATEWAY_ATOMIC_UNITS;

    private static $cryptonote;
    private static $scala_wallet_rpc;
    private static $scala_explorer_tools;
    private static $log;
    ...
    {
        return apply_filters('woocommerce_gateway_icon', '<img src="'.MONERO_GATEWAY_PLUGIN_URL.'assets/images/scala-icon.png"/>', $this->id);
    }

    function __construct($add_action=true)
    {
        $this->id = self::$_id;
        $this->method_title = __(self::$_method_title, 'scala_gateway');
        $this->method_description = __(self::$_method_description, 'scala_gateway');
        ...
        self::$address = $this->settings['scala_address'];
        ...
        self::$use_scala_price = $this->settings['use_scala_price'] == 'yes';
        self::$use_scala_price_decimals = $this->settings['use_scala_price_decimals'];
        ...
        self::$cryptonote = new Scala_Cryptonote();
        if(self::$confirm_type == 'scala-wallet-rpc') {
            require_once('class-scala-wallet-rpc.php');
            self::$scala_wallet_rpc = new Scala_Wallet_Rpc(self::$host, self::$port);
        } else {
            require_once('class-scala-explorer-tools.php');
            self::$scala_explorer_tools = new Scala_Explorer_Tools(self::$testnet);
        }
    ...
    {
        $this->form_fields = include 'admin/scala-gateway-admin-settings.php';
    }

    public function validate_scala_address_field($key,$address)
    {
        if($this->settings['confirm_type'] == 'viewkey') {
            if (strlen($address) == 95 && substr($address, 0, 1) == '4')
                if(self::$cryptonote->verify_checksum($address))
                    return $address;
            self::$_errors[] = 'Scala address is invalid';
        }
        return $address;
    }
    ...
    public function admin_options()
    {
        ...
        if($confirm_type === 'scala-wallet-rpc')
        ...
        include MONERO_GATEWAY_PLUGIN_DIR . '/templates/scala-gateway/admin/settings-page.php';
    }
    ...
    public static function admin_balance_info()
    {
        ...
        $wallet_amount = self::$scala_wallet_rpc->getbalance();
        $height = self::$scala_wallet_rpc->getheight();
        if (!isset($wallet_amount)) {
            self::$_errors[] = 'Cannot connect to scala-wallet-rpc';
            self::$log->add('Scala_Payments', '[ERROR] Cannot connect to scala-wallet-rpc');
            return array(
                'height' => 'Not Available',
                'balance' => 'Not Available',
                'unlocked_balance' => 'Not Available',
            ...
            return array(
                'height' => $height,
                'balance' => self::format_scala($wallet_amount['balance']).' Scala',
                'unlocked_balance' => self::format_scala($wallet_amount['unlocked_balance']).' Scala'
            );
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if(self::$confirm_type != 'scala-wallet-rpc') {
          ...
          // Generate subaddress
          $payment_id = self::$scala_wallet_rpc->create_address(0, 'Order: ' . $order_id);
          if(isset($payment_id['address'])) {
            $payment_id = $payment_id['address'];
          }
          else {
            self::$log->add('Scala_Gateway', 'Couldn\'t create subaddress for order ' . $order_id);
          }
        }
        ...
        if($rate != 0)
            $scala_amount = 1e8 * $fiat_amount / $rate;
        else{
            // Critical, the price has not been retrivied.
            $scala_amount = -1;
            $error_message = "The price for Scala could not be retrieved. Please contact the merchant.";
            self::$log->add('Scala_Payments', "[ERROR] Impossible to retrieve price for order: ".$order_id);
            wc_add_notice( __('Payment error:', 'woothemes') . $error_message, 'error' );
            return;
        }
        
        if(self::$discount)
            $scala_amount = $scala_amount - $scala_amount * self::$discount / 100;

        $scala_amount = intval($scala_amount * MONERO_GATEWAY_ATOMIC_UNITS_POW);

        $query = $wpdb->prepare("INSERT INTO $table_name (order_id, payment_id, currency, rate, amount) VALUES (%d, %s, %s, %d, %d)", array($order_id, $payment_id, $currency, $rate, $scala_amount));
        $wpdb->query($query);

        $order->update_status('on-hold', __('Awaiting offline payment', 'scala_gateway'));
    ...
    public static function do_update_event()
    {
        ...
        $api_link = 'https://api.coingecko.com/api/v3/simple/price?ids=stellite&vs_currencies='.$currencies;
        $curl = curl_init();
        ...
        if(isset($price)) {
            ...
            foreach($price['stellite'] as $currency=>$rate) {
                ...
                $result = $wpdb->query($query);
              	if(!$result){
                    self::$log->add('Scala_Payments', "[ERROR] Impossible to write DB. Please check your DB connection or enable Debugging.");
                }
            }
        }
        else{
             self::$log->add('Scala_Payments', "[ERROR] Unable to fetch prices from coingecko.com.");
        }
        ...
        if(self::$confirm_type == 'scala-wallet-rpc')
            ...
            $height = self::$scala_explorer_tools->getheight();
        set_transient('scala_gateway_network_height', $height);

        // Get pending payments
        $table_name_1 = $wpdb->prefix.'scala_gateway_quotes';
        $table_name_2 = $wpdb->prefix.'scala_gateway_quotes_txids';
        ...
        foreach($pending_payments as $pending) {
            ...
            $amount_scala = $quote->amount_total;
            ...
            $paid = $amount_paid > $amount_scala - MONERO_GATEWAY_ATOMIC_UNIT_THRESHOLD;
            ...
            if($paid && $confirmed) {
                self::$log->add('Scala_Payments', "[SUCCESS] Payment has been confirmed for order id $order_id and payment id $payment_id");
                ...
                if(self::is_virtual_in_cart($order_id) == true){
                    $order->update_status('completed', __('Payment has been received.', 'scala_gateway'));
                } else {
                    $order->update_status('processing', __('Payment has been received.', 'scala_gateway'));
                }

            } else if($paid) {
                self::$log->add('Scala_Payments', "[SUCCESS] Payment has been received for order id $order_id and payment id $payment_id");
                ...
                if($order_age_seconds > self::$valid_time) {
                    self::$log->add('Scala_Payments', "[FAILED] Payment has expired for order id $order_id and payment id $payment_id");
                    $query = $wpdb->prepare("UPDATE $table_name_1 SET pending=0 WHERE payment_id=%s", array($payment_id));
                    $wpdb->query($query);

                    unset(self::$payment_details[$order_id]);

                    $order->update_status('cancelled', __('Payment has expired.', 'scala_gateway'));
                }
            }
        }
    }
    ...
    {
        $txs = array();
        $address_index = self::$scala_wallet_rpc->get_address_index($subaddress);
        if(isset($address_index['index']['minor'])){
          $address_index = $address_index['index']['minor'];
        }
        else {
          self::$log->add('Scala_Gateway', '[ERROR] Couldn\'t get address index of subaddress: ' . $subaddress);
          return $txs;
        }
        $payments = self::$scala_wallet_rpc->get_transfers(array( 'in' => true, 'pool' => true, 'subaddr_indices' => array($address_index)));
    ...
    {
        $txs = array();
        $outputs = self::$scala_explorer_tools->get_outputs(self::$address, self::$viewkey);
        ...
        if (count($details)) {
            ...
            foreach($details as $tx) {
                ...
                $txs[] = array(
                    ...
                    'amount_formatted' => self::format_scala($tx->amount_paid)
                );
            ...
            if(count($heights) && !in_array(0, $heights)) {
                $height = get_transient('scala_gateway_network_height');
                $highest_block = max($heights);
            ...
            if(self::$confirm_type == 'scala-wallet-rpc') {
                ...
                if ($address) {
                    ...
                    self::$log->add('Scala_Gateway', '[ERROR] Merchant has not set Scala address');
                    return '[ERROR] Merchant has not set Scala address';
                }
            }
            ...
            $amount_formatted = self::format_scala($amount_due);
            $qrcode_uri = 'scala:'.$integrated_addr.'?tx_amount='.$amount_formatted.'&tx_payment_id='.$payment_id;
            ...
            $payment_details = array(
                ...
                'amount_total_formatted' => self::format_scala($amount_total),
                'amount_paid_formatted' => self::format_scala($amount_paid),
                'amount_due_formatted' => self::format_scala($amount_due),
                'status' => $status,
    ...
    public static function get_payment_details_ajax() {
        ...
        if(isset($_GET['order_id'])){
            ...
            if($order->get_payment_method() != self::$_id)
                self::ajax_output(array('error' => '[ERROR] Order not paid for with Scala'));
    ...
    public static function admin_order_page($post)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include MONERO_GATEWAY_PLUGIN_DIR . '/templates/scala-gateway/admin/order-history-error-page.php';
            return;
        }
        include MONERO_GATEWAY_PLUGIN_DIR . '/templates/scala-gateway/admin/order-history-page.php';
    }
    ...
    public static function customer_order_page($order)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include MONERO_GATEWAY_PLUGIN_DIR . '/templates/scala-gateway/customer/order-error-page.php';
            return;
        }
        $show_qr = self::$show_qr;
        $details_json = json_encode($details);
        $ajax_url = WC_AJAX::get_endpoint('scala_gateway_payment_details');
        include MONERO_GATEWAY_PLUGIN_DIR . '/templates/scala-gateway/customer/order-page.php';
    }
    ...
    public static function customer_order_email($order)
    {
        ...
        $details = self::get_payment_details($order_id);
        if(!is_array($details)) {
            include MONERO_GATEWAY_PLUGIN_DIR . '/templates/scala-gateway/customer/order-email-error-block.php';
            return;
        }
        include MONERO_GATEWAY_PLUGIN_DIR . '/templates/scala-gateway/customer/order-email-block.php';
    }
    ...
    public static function use_scala_price()
    {
        return self::$use_scala_price;
    }
    ...
    {
        $rate = self::get_live_rate($currency);
        $scala_amount = intval(MONERO_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / MONERO_GATEWAY_ATOMIC_UNITS_POW;
        $scala_amount_formatted = sprintf('%.'.self::$use_scala_price_decimals.'f', $scala_amount);
...
HTML;
    ...
    public static function convert_wc_price_order($price_html, $order)
    {
        ...
        $scala_amount = intval(MONERO_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / MONERO_GATEWAY_ATOMIC_UNITS_POW;
        $scala_amount_formatted = sprintf('%.'.MONERO_GATEWAY_ATOMIC_UNITS.'f', $scala_amount);
...
HTML;
    ...
    public static function format_scala($atomic_units) {
